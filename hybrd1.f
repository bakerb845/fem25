...
C***END PROLOGUE  FZERO
      REAL*8 A,ACBS,ACMB,AE,AW,B,C,CMB,ER,FA,FB,FC,FX,FZ,P,Q,R,
     +     RE,RW,T,TOL,Z
      INTEGER*4 IC,IFLAG,KOUNT
...
C
      ...
      FB = F(T)
      KOUNT = 2
      IF (DSIGN(1.0D0,FZ) .EQ. DSIGN(1.0D0,FB)) GO TO 1
      C = Z
      GO TO 2
    1 IF (Z .EQ. C) GO TO 2
      T = C
      FC = F(T)
      KOUNT = 3
      IF (DSIGN(1.0D0,FZ) .EQ. DSIGN(1.0D0,FC)) GO TO 2
      B = Z
      FB = FZ
    2 A = C
      FA = FC
...
C
    3 IF (DABS(FC) .GE. DABS(FB)) GO TO 4
C
...
C
      IF (DSIGN(1.0D0,FB) .NE. DSIGN(1.0D0,FC)) GO TO 3
      C = A
      FC = FA
      GO TO 3
C
