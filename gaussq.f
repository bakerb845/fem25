...
c
      subroutine gaussq(kind, n, alpha, beta, kpts, endpts, t, w)
c
...
c
      double precision t(n), w(n), endpts(2), muzero, t1,
...
c
      ...
      double precision abi, a2b2, gamma, pi, dsqrt, ab
c
...
c
   10 muzero = 2.0d0
      do 11 i = 1, nm1
         a(i) = 0.0d0
         abi = dfloat(i)
   11    b(i) = abi/dsqrt(4.d0*abi*abi - 1.0d0)
      a(n) = 0.0d0
      return
c
...
c
   50 ab = alpha + beta
     ...
     1  abi*abi))
      ...
      do 51 i = 2, nm1
         abi = 2.0d0*dfloat(i) + ab
         a(i) = a2b2/((abi - 2.0d0)*abi)
   51    b(i) = dsqrt (4.0d0*dfloat(i)*(dfloat(i) + alpha)*(dfloat(i) 
     ;               + beta)*(dfloat(i) + ab)/
     1   ((abi*abi - 1.d0)*abi*abi))
      abi = 2.0d0*dfloat(n) + ab
...
c
   60 muzero = gamma(alpha + 1.0d0)
      do 61 i = 1, nm1
         a(i) = 2.0d0*dfloat(i) - 1.0d0 + alpha
   61    b(i) = dsqrt(dfloat(i)*(dfloat(i) + alpha))
      a(n) = 2.0d0*dfloat(n) - 1.d0 + alpha
      return
      end
c
c
